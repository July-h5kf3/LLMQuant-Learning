...
def _box_iou(box: torch.Tensor, boxes: torch.Tensor) -> torch.Tensor:
    ...
    box_area_value = ((box[2] - box[0]).clamp(min=0) * (box[3] - box[1]).clamp(min=0))
    boxes_area = box_area(boxes)
    union = box_area_value + boxes_area - inter_area
...
def batched_nms(
    ...
    for class_id in torch.unique(idxs):
        ...
        while order.numel() > 0:
            current = order[0]
            current_index = class_indices[current]
            ...
            remaining = order[1:]
            ious = _box_iou(current_box, class_boxes[remaining])
            order = remaining[ious <= iou_threshold]

    if not keep:
        return torch.empty((0,), dtype=torch.long, device=boxes.device)

    keep_tensor = torch.stack(keep)
